defmodule Forth do
  ...
  def eval(%Forth{} = ev, s) do
      ...
      |> Enum.map(&try_parsing_integer/1) # convert what we can to ints
      |> evaluate(ev)
  end

  defp evaluate([], %Forth{} = ev), do: ev

  # Until Elixir/Erlang support matching maps in function definitions, had to add struct field for whether instruction was in definitions
  ...
  defp try_parsing_integer(s), do: try_parsing_integer(s, Integer.parse(s))
